...
class SGD(Model):
    ...
    def evaluate(self):
        ...
        print('inverted values: {:.2e} {:.2e}'.format(E_f, self.nu_f))
...
#     # not to be mistaken with the Generator class
#     force = torch.DoubleTensor([[params.force]] * params.batch_size_start)
#     E_r = params.E_0 / 4 * np.random.randn() + params.E_0
#     nu_r = params.nu_0 / 4 * np.random.randn() + params.nu_0
#     mu, beta = lame(E_r,nu_r)

#     return [mu, beta, force]
# class Model:
#     def __init__(self, params):
#         # values to store
#         self.mu, self.beta, self.force = Generator_func(params, params.generate_samples_mode)
#         self.mu = torch.tensor([[self.mu]], requires_grad=True, dtype=torch.float64)
#         self.beta = torch.tensor([[self.beta]], requires_grad=True, dtype=torch.float64)
#         self.history = np.zeros([0,3])
...
#     def __init__(self, params, eng):
#         super().__init__(params)
#         self.optimizer = torch.optim.Adam([self.mu, self.beta], lr=params.lr, betas=(params.beta1, params.beta2))
#         self.loss = torch.nn.MSELoss()
